...
export default function LoanPayment() {
  ...
  // Filter only approved loans of the logged-in user
  const userApprovedLoans = loans.filter(
  (loan) => loan.userId === loggedInUser?.id && loan.status === "approved"
);

  // Calculate total outstanding (principal + interest - amountPaid)
const totalOutstanding = userApprovedLoans.reduce((sum, loan) => {
    ...
    <div className="min-h-screen bg-[#00353B] p-4 font-poppins relative">
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="w-full max-w-2xl bg-[#00353B] text-white rounded-2xl shadow-lg p-6 sm:p-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-center text-[#F9A825] mb-10">Loan Payment</h2>
          <p className="font-semibold text-[#FFFFFF] mb-6 text-2xl sm:text-base">
            Make payment toward your loan balance
          </p>

          <div className="mb-4">
            <label className="block text-xl font-semibold text-[#FFFFFF] mb-4">Loan Amount Remaining</label>
            <input
              type="text"
              value={`Frw ${totalOutstanding.toLocaleString()}`}
              readOnly
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center border border-[#F9A825] rounded-lg px-3 py-2 bg-transparent">
              ...
              <input type="date" className="w-full bg-transparent text-white focus:outline-none" />
            </div>
          </div>

          <div className="mb-6">
            <label className="block text-xl font-semibold text-[#FFFFFF] mb-4">
              Payment Method <span className="bg-gradient-to-r from-[#545D5E] to-[#B0C2C4] bg-clip-text text-transparent">(Optional)</span>
            </label>
            <div className="flex items-center border border-[#F9A825] rounded-lg px-3 py-2 bg-transparent">
