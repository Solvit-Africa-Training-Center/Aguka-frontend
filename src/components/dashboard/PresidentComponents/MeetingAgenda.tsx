...
const MeetingAgenda: React.FC<MeetingAgendaProps> = ({ groupId }) => {
  ...
  const { data, isLoading, isError } = useGetAnnouncementsQuery({ page: 1, limit: 100 });
  ...
  if (isError)
    return <p className="text-white text-center mt-10">Failed to load meetings.</p>;
    ...
    <div className="p-6 bg-[#003B42] text-white rounded-2xl shadow-lg max-w-6xl mx-auto mt-6 border-b-4 border-l-4 border-[#F9A825] font-poppins">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="border border-[#F9A825] mt-2 text-[#F9A825] px-4 py-1 rounded-full text-sm hover:bg-[#F9A825] hover:text-[#00353B] transition"
        >
          + Schedule meeting
        </button>
      </div>
      ...
      {filteredAnnouncements.length > 0 ? (
        <div className="overflow-y-auto max-h-[500px] space-y-6">
          {filteredAnnouncements.map((m: Announcement, idx: number) => (
            <div key={idx} className="flex flex-col gap-3 bg-[#004045] rounded-xl p-4 shadow">
              {/* Title */}
